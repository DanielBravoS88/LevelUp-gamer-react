      ...
      {items.map(p => {
        return (
          <article className="card" key={p.name}>
            <Link className="media" to={`/producto/${slugify(p.name)}`}>
              <span className="tag">{p.category}</span>
              <img 
                src={p.image || '/img/placeholder.jpg'} 
                alt={p.name} 
                loading="lazy"
                onError={(e) => {
                  console.error(`❌ Error cargando imagen: ${p.image}`);
            ...
            <div className="body">
              <h3 className="title">
                <Link to={`/producto/${slugify(p.name)}`} style={{color:'inherit'}}>
                  {p.name}
                </Link>
              </h3>
